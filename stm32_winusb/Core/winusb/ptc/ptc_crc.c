...
// The checksum table generate by: https://github.com/pnt325/crc
...
uint16_t ptc_crc_calc(const uint8_t *data, uint16_t len, uint16_t crc_init) {
	...
	for (uint16_t i = 0; i < len; i++) {
		data_index = (crc_val & 0xff) ^ data[i];
		crc_val = (crc_val >> 8) ^ crc16_table[data_index];
	}

	return crc_val ^ FINAL_XOR_VALUE;
}
